...
export default function EditMenuItem() {
  ...
  const existing = MENU_ITEMS.find(i => i._id === id || i.id === id);
  ...
  useEffect(() => {
    if (existing) {
      setForm({
        ...
        category: existing.category || 'Mains',
  ...
  if (!existing) {
    return (
      <div className="flex min-h-screen bg-ivory items-center justify-center">
        <p className="text-sage">Item not found.</p>
        <button onClick={() => navigate('/admin/menu-items')} className="ml-4 text-forest hover:underline">
          Go back
        </button>
      </div>
    );
    ...
    <div className="flex min-h-screen bg-ivory">
      ...
      <div className="flex-1 flex flex-col lg:ml-56">
        <AdminTopbar
          ...
          subtitle={`Editing: ${existing.name}`}
